...
GEMINI_API_KEY = "AIzaSyDOkA0EVVvKZgJdGpXusECLkfgumgpqf40"
...
def _get_subfield_language_resources(sub_field_data: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for lang in sub_field_data.get("languages", []):
        resources.append({
            "title": f"{lang['name']} documentation — {sub_field_data.get('name', 'Specialization')}",
            "url": lang["link"],
        })
    return resources
...
def _fetch_real_resources_with_gemini(
    ...
    cache_key = f"{career_name}:::{phase_name}:::{learning_path}:::{is_job_phase}:::{is_first_phase}:::{experience_level}:::{sub_field_name}:::{selected_language}:::{prefer_paid}"
    ...
    # Build cost/pricing context
    cost_ctx = ""
    if prefer_paid is True:
        cost_ctx = "\nThe user PREFERS PAID, structured courses. Prioritize paid certifications, premium Coursera/Udemy courses, bootcamps, and professional training programs. Include pricing info in titles when possible."
    elif prefer_paid is False:
        cost_ctx = "\nThe user wants ONLY FREE resources. Prioritize freeCodeCamp, YouTube tutorials, free Coursera audit mode, Khan Academy, W3Schools, MDN Web Docs, and open-source documentation. Do NOT suggest paid courses or certifications that require payment."
...
Key skills for this career: {skills_str or 'N/A'}{task_ctx}{level_ctx}{cost_ctx}
...
The key skills for this career are: {skills_str or 'N/A'}{task_ctx}{level_ctx}{cost_ctx}
...
def generate_steps_for_career(career: Dict[str, Any], profile_skills: List[str], focus: str, beginner: bool = True, experience_level: str = "", sub_field: str = "", selected_language: str = "", prefer_paid: bool = None):
...
def generate_distinct_roadmaps(profile: Dict[str, Any], top_career: Dict[str, Any]):
    ...
    prefer_paid = profile.get('prefer_paid', None)
...
def get_recommended_fields(career_name: str, language: str) -> List[Dict[str, str]]:
    """Recommend 3-5 related career fields from the dataset based on shared skills and languages.
    
    Uses the actual categorized_careers_by_education.json dataset to find careers that:
    1. Share skills with the selected career
    2. Use the same language/tool the user selected
    3. Are in the same or adjacent tech categories
    
    Returns a list of dicts with 'field' and 'description' keys.
    """
    cache_key = f"recommend:::{career_name}:::{language}"
    if cache_key in _RESOURCE_CACHE:
        return _RESOURCE_CACHE[cache_key]
    
    # Load the dataset
    from pathlib import Path
    dataset_path = Path('data') / 'categorized_careers_by_education.json'
    ...
    # Build a flat list of all careers with metadata
    all_careers = []
    source_career = None
    seen_names = set()
    
    for cat_block in raw:
        cat_name = cat_block.get('category', '')
        for lp_type in ['self_study', 'university']:
            for c in cat_block.get('careers_by_learning_path', {}).get(lp_type, []):
                cname = c.get('career', '').lower().strip()
                if cname in seen_names:
                    continue
                seen_names.add(cname)
                
                skills_set = set(s.lower().strip() for s in c.get('skills', []))
                lang_names = set()
                # Languages from top-level
                for lang in c.get('languages', []):
                    lang_names.add(lang.get('name', '').lower().strip())
                # Languages from sub_fields
                for sf in c.get('sub_fields', []):
                    for lang in sf.get('languages', []):
                        lang_names.add(lang.get('name', '').lower().strip())
                ...
                entry = {
                    ...
                    'has_sub_fields': bool(c.get('sub_fields')),
                }
                
                if cname == career_name.lower().strip():
                    source_career = entry
                else:
                    # Skip umbrella careers that have sub_fields (e.g. "software developer")
                    # since they are broad parent categories, not specific career paths
                    if not c.get('sub_fields'):
                        all_careers.append(entry)
    
    if not source_career:
        return []
    
    # Score each career by relevance to the source career + selected language
    lang_lower = language.lower().strip()
    scored = []
    
    for candidate in all_careers:
        score = 0.0
        reasons = []
        
        # Shared skills (most important signal)
        shared_skills = source_career['skills'] & candidate['skills']
        if shared_skills:
            score += len(shared_skills) * 3
            reasons.append(f"shares {len(shared_skills)} skill(s)")
        
        # Candidate uses the same language the user selected
        if lang_lower in candidate['languages']:
            score += 5
            reasons.append(f"also uses {language}")
        
        # Any shared languages/tools
        shared_langs = source_career['languages'] & candidate['languages']
        if shared_langs:
            score += len(shared_langs) * 2
        
        # Same category bonus
        if candidate['category'] == source_career['category']:
            score += 2
            reasons.append("same field")
        
        # Adjacent tech categories bonus
        tech_cats = {
        ...
        if source_career['category'] in tech_cats and candidate['category'] in tech_cats:
            if candidate['category'] != source_career['category']:
                score += 1  # Small bonus for adjacent tech category
        
        # Skip careers with zero relevance
        if score <= 0:
            continue
        
        # Build a human-readable description
        if reasons:
            desc = f"Great next step — {', '.join(reasons)}. Complements your {career_name} experience."
        else:
            desc = f"A related career that builds on your {career_name} background."
        
        scored.append({
            'field': candidate['career'],
            'description': desc,
            'score': score,
        })
    ...
    results = [{'field': s['field'], 'description': s['description']} for s in scored[:5]]
    
    _RESOURCE_CACHE[cache_key] = results
    return results
