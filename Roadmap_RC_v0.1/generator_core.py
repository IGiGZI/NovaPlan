...
GEMINI_API_KEY = "AIzaSyDGOQZiHMhXHxVpjhHRsKVTVmYIClqqCbM"
...
def _fetch_real_resources_with_gemini(
    ...
    cache_key = f"{career_name}:::{phase_name}:::{learning_path}:::{is_job_phase}:::{is_first_phase}:::{experience_level}:::{sub_field_name}"
...
- ALWAYS use search/listing page URLs instead of specific course pages. Examples:
  - GOOD: "https://www.coursera.org/search?query={career_name.replace(' ', '+')}+certificate{level_search_suffix}"
  - GOOD: "https://www.udemy.com/courses/search/?q={career_name.replace(' ', '+')}{level_search_suffix}"
...
- ALWAYS use search/listing page URLs on major platforms. Examples:
  - GOOD: "https://www.coursera.org/search?query={career_name.replace(' ', '+')}+{skills_str.split(',')[0].strip().replace(' ', '+') if skills_str else ''}{level_search_suffix}"
  - GOOD: "https://www.youtube.com/results?search_query={career_name.replace(' ', '+')}+tutorial{level_search_suffix}"
...
def generate_steps_for_career(career: Dict[str, Any], profile_skills: List[str], focus: str, beginner: bool = True, experience_level: str = "", sub_field: str = "", selected_language: str = ""):
    ...
    for i, (phase_name, task_text) in enumerate(phases):
        ...
        if is_first_phase and sub_field_data:
