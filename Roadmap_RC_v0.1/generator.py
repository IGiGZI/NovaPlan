...
def suggest_career_from_summary(user_summary: str) -> list[dict]:
    ...
    from generator_core import client

    # Use Gemini to extract professional keywords and domains from natural text
    try:
        prompt = f"""
You are an expert career counselor. A user has provided a description of themselves, their interests, hobbies, and skills.
Extract the core skills, professional interests, and relevant career domains from their description.
Output a concise list of keywords and professional terms that best represent their profile, which will be used to search a career database.
Do not include conversational filler. Just the extracted keywords and concepts.
...
"""
        ...
        search_query = f"{user_summary}. {enriched_summary}"
    except Exception as e:
        print(f"WARNING: Gemini enrichment failed for summary: {e}")
        search_query = user_summary
    ...
    # Query the career index with the enriched self-description
    results = query_career(search_query, k=5)
