...
GEMINI_API_KEY = "AIzaSyBVKrxlKY7YnEJcPQKf5CoyqqTRXer0ORw"
...
def _fetch_real_resources_with_gemini(
    ...
    cache_key = f"{career_name}:::{phase_name}:::{learning_path}:::{is_job_phase}:::{is_first_phase}"
...
I need real Egyptian universities and programs for someone pursuing a career as a {career_name}.
Key skills for this career: {skills_str or 'N/A'}{task_ctx}

Provide 3-5 resources that are ONLY from Egypt:
...
The key skills for this career are: {skills_str or 'N/A'}{task_ctx}

For the "{phase_name}" phase, I need real resources available in Egypt.

Provide 3-5 resources. ONLY use well-known platforms and institutions where you are confident the URL exists:
...
CRITICAL RULES:
- Every URL must be a REAL, WORKING link. Do NOT invent or guess URLs.
- Resources MUST be specifically relevant to {career_name}, NOT generic or for a different career.
- ALWAYS use search/listing page URLs on major platforms. Examples:
  - GOOD: "https://www.coursera.org/search?query=helicopter+pilot"
  - GOOD: "https://www.udemy.com/courses/search/?q=helicopter+flight"
  - BAD: "https://www.coursera.org/learn/helicopter-flight-training-2024" (may not exist)
  - BAD: "https://www.some-random-academy.com" (domain may not exist)
...
For the "{phase_name}" phase, I need 3-5 real, high-quality certification courses or bootcamps that are SPECIFICALLY for becoming a {career_name}.
...
CRITICAL RULES:
- Every URL must be a REAL, WORKING link on one of these platforms.
- Resources MUST be specifically for {career_name}, NOT for a different career.
- Focus on certifications that teach {skills_str or career_name}.
- ALWAYS use search/listing page URLs instead of specific course pages. Examples:
  - GOOD: "https://www.coursera.org/search?query={career_name.replace(' ', '+')}+certificate"
  - GOOD: "https://www.udemy.com/courses/search/?q={career_name.replace(' ', '+')}"
  - BAD: "https://www.coursera.org/learn/some-specific-course" (may not exist)
...
For the "{phase_name}" phase, I need 3-5 real, high-quality resources (courses, tutorials, documentation, tools) that help someone develop {career_name} skills.
...
CRITICAL RULES:
- Every URL must be a REAL, WORKING link. Do NOT invent or guess URLs.
- Resources MUST be specifically relevant to {career_name} and the skills: {skills_str}.
- Do NOT return resources for a different career.
- ALWAYS use search/listing page URLs on major platforms. Examples:
  - GOOD: "https://www.coursera.org/search?query={career_name.replace(' ', '+')}+{skills_str.split(',')[0].strip().replace(' ', '+') if skills_str else ''}"
  - GOOD: "https://www.youtube.com/results?search_query={career_name.replace(' ', '+')}+tutorial"
  - BAD: "https://www.coursera.org/learn/specific-course-name" (may not exist)
...
def generate_steps_for_career(career: Dict[str, Any], profile_skills: List[str], focus: str, beginner: bool = True):
    ...
    if focus == "Fast-Entry" and len(phases) >= 7:
        # Condense 7 phases into 4 by merging related ones:
        #   1. Foundation  = Certification/Degree (phase 0) + Core Skill Learning (phase 1)
        #   2. Skill Building = Intermediate/Advanced (phase 2) + Hands-on Practice (phase 3)
        #   3. Professional Profile = Portfolio (phase 4) + CV/Profile (phase 5)
        #   4. Job Application = Job Application (phase 6)
        merged_phases = [
    ...
    for i, (phase_name, task_text) in enumerate(phases):
        ...
        # Fetch resources from Gemini with career context
        career_skills = career.get('skills', [])
        real_resources = _fetch_real_resources_with_gemini(
            career_name, phase_name, learning_path,
...
def generate_distinct_roadmaps(profile: Dict[str, Any], top_career: Dict[str, Any]):
    """Generate two roadmap variants: Depth (full 7-phase) and Fast-Entry (condensed 4-phase)."""
    focuses = ['Depth', 'Fast-Entry']
    roadmaps = []
    for f in focuses:
        steps = generate_steps_for_career(top_career, profile.get('skills', []), f, beginner=True)
        rm = Roadmap(
            path_title=top_career.get('career') or top_career.get('label', 'Career'),
            focus=f,
