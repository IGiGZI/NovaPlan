...
def _load_career_dataset(path: Path) -> List[Dict[str, Any]]:
    ...
    # If the file is already a flat list of careers, just return it
    if isinstance(raw, list) and (not raw or 'careers_by_education' not in raw[0]):
        return raw
    ...
    if isinstance(raw, list):
        for category_block in raw:
            ...
            careers_by_education = category_block.get('careers_by_education') or {}
            for edu_level, careers in careers_by_education.items():
                for c in careers or []:
                    item = dict(c)
                    item.setdefault('category', category_name)
                    item.setdefault('education_level', edu_level)
                    flat.append(item)
    return flat
