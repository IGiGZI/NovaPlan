                  ...
                  <ul className="list-disc list-inside text-gray-300 space-y-2">
                    {selectedNode.data.milestones.map((milestone, i) => (
                      <li key={i} className="flex flex-col">
                        <span className="font-bold text-white">{typeof milestone === 'string' ? milestone : milestone?.title}</span>
                        {typeof milestone !== 'string' && milestone?.description && (
                          <span className="text-sm pl-5 border-l-2 border-purple-500/30 mt-1 text-gray-400">{milestone.description}</span>
                        )}
                      </li>
                    ))}
                  </ul>
                    ...
                    {selectedNode.data.resources.map((resource, i) => (
                      <li key={i}>
                        {typeof resource === 'string' ? (
                          ...
                          <a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 hover:underline">
                            {resource.title}
                          </a>
                        )}
                      </li>
